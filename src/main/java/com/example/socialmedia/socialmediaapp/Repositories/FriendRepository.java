...
public interface FriendRepository extends JpaRepository<Friends, BigInteger> {
        ...
        @Query(value = "select ifnull(allRecords.id,NULL) as id,ifnull(allRecords.first_name,NULL) as first_name,ifnull(allRecords.last_name,NULL) as last_name,ifnull(allRecords.profile_photo,NULL) as profile_photo,ifnull(allRecords.messageSend,NULL) as messageSend,ifnull(allRecords.typeofMessage,NULL) as typeofMessage,ifnull(allRecords.seen,NULL) as seen from ( select finalRecords.*,messages.messageSend,messages.seen,1 as typeofMessage from( select records.*,max(messages.id) as messageId from ( select users.* from messages inner join users on users.id=messages.senderId where messages.recieverId = :userid  group by users.id ) as records inner join messages on messages.senderId=records.id where messages.recieverId = :userid group by records.id ) as finalRecords inner join messages on messages.id=finalRecords.messageId union all select finalRecords.*,messages.messageSend,messages.seen,0 as typeofMessage from ( select records.*,max(messages.id) as messageId from ( select users.* from messages inner join users on users.id=messages.recieverId where messages.senderId = :userid group by users.id ) as records inner join messages on messages.recieverId=records.id where messages.senderId = :userid group by records.id ) as finalRecords inner join messages on finalRecords.messageId=messages.id ) as allRecords", countQuery = "SELECT COUNT(*) from (select ifnull(allRecords.id,NULL) as id,ifnull(allRecords.first_name,NULL) as first_name,ifnull(allRecords.last_name,NULL) as last_name,ifnull(allRecords.profile_photo,NULL) as profile_photo,ifnull(allRecords.messageSend,NULL) as messageSend,ifnull(allRecords.typeofMessage,NULL) as typeofMessage,ifnull(allRecords.seen,NULL) as seen from ( select finalRecords.*,messages.messageSend,messages.seen,1 as typeofMessage from( select records.*,max(messages.id) as messageId from ( select users.* from messages inner join users on users.id=messages.senderId where messages.recieverId = :userid  group by users.id ) as records inner join messages on messages.senderId=records.id where messages.recieverId = :userid group by records.id ) as finalRecords inner join messages on messages.id=finalRecords.messageId union all select finalRecords.*,messages.messageSend,messages.seen,0 as typeofMessage from ( select records.*,max(messages.id) as messageId from ( select users.* from messages inner join users on users.id=messages.recieverId where messages.senderId = :userid group by users.id ) as records inner join messages on messages.recieverId=records.id where messages.senderId = :userid group by records.id ) as finalRecords inner join messages on finalRecords.messageId=messages.id ) as allRecords) as total", nativeQuery = true)
