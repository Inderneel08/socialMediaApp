...
public interface UserRepository extends JpaRepository<Users, BigInteger> {
    ...
    // @Query(value = "SELECT users.*,CASE WHEN f.senderId = :userid THEN CASE WHEN
    // f.current_status=1 THEN 1 ELSE 0 END ELSE -1 END AS approved from users LEFT
    ...
    @Query(value = "SELECT users.*,CASE WHEN friends.current_status is NULL THEN -1 ELSE CASE WHEN friends.current_status=1 THEN 1 ELSE 0 END END as approved  FROM friends RIGHT JOIN users on friends.recieverId=users.id and friends.senderId = :userid where users.id != :userid", nativeQuery = true)
